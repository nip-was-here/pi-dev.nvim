...
cat > "$tmp_lua" <<'LUA'
require('pi-dev').setup({ keymaps = { enable = false }, ui = { width = 80, input_height = 10, render = { fold_tool_output_over = 20, fold_thinking_over = 3 } } })
...
local function assert_thinking_fold(header, body, should_close, label)
  ...
  if should_close then
    assert(foldclosed(header + 1) ~= -1, label .. ': thinking detail over the configured threshold should auto-fold')
    assert(foldclosed(body_line) ~= -1, label .. ': long thinking body should be hidden')
  else
    assert(foldclosed(header + 1) == -1, label .. ': thinking detail at/below the configured threshold should stay open')
    assert(foldclosed(body_line) == -1, label .. ': short thinking body should remain visible')
  end
end

renderer.render_messages({
  { role = 'assistant', content = { { type = 'thinking', thinking = thinking_text('restored short', 2) }, { type = 'text', text = 'after restored short' } } },
  { role = 'assistant', content = { { type = 'thinking', thinking = thinking_text('restored long', 3) }, { type = 'text', text = 'after restored long' } } },
...
assert_thinking_fold(restored_short_header, 'restored short 2', false, 'restored short')
assert_thinking_fold(restored_long_header, 'restored long 3', true, 'restored long')

renderer.clear('Pi.dev thinking fold threshold live short')
renderer.handle_event({ type = 'message_start', message = { role = 'assistant' } })
renderer.handle_event({ type = 'message_update', assistantMessageEvent = { type = 'thinking_delta', delta = thinking_text('live short', 2) } })
...
assert_thinking_fold(live_short_header, 'live short 2', false, 'live short')

renderer.clear('Pi.dev thinking fold threshold live long')
renderer.handle_event({ type = 'message_start', message = { role = 'assistant' } })
renderer.handle_event({ type = 'message_update', assistantMessageEvent = { type = 'thinking_delta', delta = thinking_text('live long', 3) } })
...
assert_thinking_fold(live_long_header, 'live long 3', true, 'live long')
LUA
